...
> PERSONALIZE: every `jacobbanghart.com` hostname and `jacobmbanghart@gmail.com`
> email in this section is this deployment's own domain/identity, given as
> a worked example -- substitute your own throughout (see the manifests
> under `clusters/k8s-homelab/vault/`, which have matching `PERSONALIZE`
> comments).
...
> `auth.jacobbanghart.com`, and `jacobmbanghart@gmail.com` below are this
> deployment's own domain/identity -- substitute your own (see
> `clusters/k8s-homelab/monitoring/release.yaml`'s `PERSONALIZE` comment).
